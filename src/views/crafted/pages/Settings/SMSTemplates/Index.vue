<template>
  ...
  <div>
    <div class=" d-flex flex-stack">
      <KTToolbar/>
      <button class="btn btn-primary btn-sm" @click="$router.push({name:'create_sms_template'})">
          ...
          <div class="table-responsive">
            <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer table-striped">
              ...
              <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase">
                ...
                <th class="text-start">{{ $t("message") }}</th>
              ...
              <template v-for="(item,itemKey) in items" :key="itemKey">
                <tr class="border-bottom border-gray-300 border-bottom-dashed"
                  ...
                  <td class="p-1">
                    <a class="menu-link px-3" href="javascript:void(0)" @click="deleteRow(item,itemKey)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchRequestApi = () => {
      ...
      store.dispatch("moduleSmsTemplate/getAllSmsTemplates", payload)
          .then(({data}) => {
            actionLoader("hide");
    ...
    const deleteRow = (item, key) => {
      Swal.fire({
        ...
        preConfirm: (result) => {
          ...
          state.loading = true;
          return store.dispatch("moduleSmsTemplate/deleteSmsTemplate", payload)
              .then(({data}) => {
                actionLoader("hide");
