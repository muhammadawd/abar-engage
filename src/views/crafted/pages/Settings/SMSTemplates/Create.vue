            ...
            <div class="card-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-2">
                  ...
                  <Field :rules="{required:true}"
                         as="textarea" rows="6"
                         type="text" v-model="model.message"
                         :name="`message`"
                         class="form-control mb-3 mb-lg-0"
                         :label="$t('message')" :placeholder="$t('message')"/>
                  <div class="fv-plugins-message-container">
                    <div class="fv-help-block">
...
export default defineComponent({
  ...
  setup() {
    ...
    const onSubmit = (values, form) => {
      ...
      state.loading = true;
      return store.dispatch("moduleSmsTemplate/createSmsTemplate", payload).then(({data}) => {
        state.loading = false;
        showMessage("success", i18n.t("success"), i18n.t("success_msg"));
        router.push({name: "list_sms_templates"});
      }).catch((response) => {
        state.loading = false;
