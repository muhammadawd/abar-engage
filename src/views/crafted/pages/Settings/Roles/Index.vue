<template>
  ...
  <div>
    <div class=" d-flex flex-stack">
      <KTToolbar/>
      <button class="btn btn-primary btn-sm" @click="$router.push({name:'create_role'})">
          ...
          <div class="table-responsive">
            <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer table-striped">
              ...
              <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase">
                ...
                <th class="text-start">{{ $t("permissions_count") }}</th>
              ...
              <template v-for="(item,itemKey) in items" :key="itemKey">
                <tr class="border-bottom border-gray-300 border-bottom-dashed"
                  ...
                  <td>
                    <b>{{ (item.permissions || []).length }} <small class="text-muted">{{$t('permissions')}}</small></b>
                  </td>
                  <!--                  <td class="p-0">-->
                      ...
                      <div
                        ...
                        <div class="menu-item px-3">
                          <a class="menu-link px-3"
                             @click="$router.push({name:'edit_role',params:{id:item.id}})">
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchRequestApi = () => {
      ...
      store.dispatch("moduleRole/getAllRoles", payload)
          .then(({data}) => {
            actionLoader("hide");
    ...
    const deleteRow = (item, key) => {
      Swal.fire({
        ...
        preConfirm: (result) => {
          ...
          state.loading = true;
          return store.dispatch("moduleRole/deleteRole", payload)
              .then(({data}) => {
                actionLoader("hide");
